...
**Date:** 2026-06-30
**Status:** BUILT 2026-07-01 (core + Optional B digest line; Optional A retry deferred). Grok-reviewed — see `-review-grok.md`; the secret-redaction blocker + non-API-cause / jq-fallback majors were folded in. **Follow-up done:** CSF/ipset (direct-plane) `failed` rows now capture a cause too (command stderr / "not found"), so the channel-agnostic slot is fully realized.
