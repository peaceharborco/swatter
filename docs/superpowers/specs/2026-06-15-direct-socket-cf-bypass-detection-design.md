...
**Observed incident (2026-06-14):** `198.51.100.23` (DigitalOcean) POSTed
`//xmlrpc.php` to the origin IP `203.0.113.10:443` with `Host: example.com`
...
`203.0.113.10:443 ← 198.51.100.23`. Resolved manually with `csf -d`.
...
- Ships as ordinary code; deployed to prod the usual way
  (`bash install/install.sh remote <sshdest>`), which preserves the server
