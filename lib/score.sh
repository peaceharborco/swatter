...
_swatter_audit() {
    ...
    printf '{"ts":%s,"iso":"%s","ip":"%s","score":%s,"action":"%s","channel":"%s","ttl":%s,"reason":"%s","reputation":%s,"mode":"%s","evidence":%s}\n' \
        "$now" "$(ts)" "$1" "$2" "$3" "$4" "${5:-0}" "$reason" "${8:-0}" "${SWATTER_MODE}" "$7" \
...
_swatter_execute_block() {
    ...
    # Strict validity gate, FIRST. score.awk's charset pre-filter admits tokens
    # like 999.999.999.999 or '::::' (harmless bytes, but not addresses) — those
    # must never reach csf/ipset/the CF API as a block target. Audit the skip
    # (parity with the never-block exempt path) so a malformed token that reached
    # scoring is visible to `why`/report, not just a stderr line.
    if ! swatter_is_valid_ip_or_cidr "$ip"; then
        log_warn "refusing to act on malformed ip token '${ip}'"
        _swatter_audit "$ip" "$folded" "skipped-invalid" "none" 0 "malformed_ip ${reason}" "$ev" "$rep"
        return 1
    fi
