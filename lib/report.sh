...
# scanner-induced fatals, under both the repeat and spread gates, are not outages),
...
_report_grade() {
    ...
    (( fsc > 0 )) && nofatal="No genuine fatals — ${fsc} scanner-induced (under the repeat and account-spread gates, not an outage)"
...
_report_summary_errors() {
    ...
    if (( f > 0 )); then
        ...
        if (( enf >= ${REPORT_GRADE_C_ERRORS:-100} )); then
            echo "${fsc} scanner-induced fatal${fscs} (under the repeat and account-spread gates — not an outage), plus ${enf} non-fatal errors — volume above routine, worth a look."
        else
            echo "${fsc} scanner-induced fatal${fscs} — under the repeat and account-spread gates, not an outage. The rest handled cleanly."
        fi
    elif (( e > 0 )); then
